...
sealed abstract class Objective[A] {
  def violations: List[Constraint[A,Double]] =
    this match {
      case Single(_, v) => v
      case Multi(xs)    => xs.foldLeft(List.empty[Constraint[A,Double]])(_ ++ _.v)
    }

  def violationCount =
    violations.length
}
final case class Single[A](f: Fit, v: List[Constraint[A, Double]]) extends Objective[A]
