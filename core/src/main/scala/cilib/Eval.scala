...
object Eval {
  private final case class Unconstrained[F[_],A](run: F[A] => Double, F: Input[F]) extends Eval[F,A]
  private final case class Constrained[F[_],A](run: F[A] => Double, cs: List[Constraint[A]], F: Input[F]) extends Eval[F,A]

  def unconstrained[F[_]: Input,A](f: F[A] => Double)(implicit F: Input[F]): Eval[F,A] =
    Unconstrained(f, F)

  def constrained[F[_]: Input,A](f: F[A] => Double, cs: List[Constraint[A]])(implicit F: Input[F]): Eval[F,A] =
    Constrained(f, cs, F)
}
