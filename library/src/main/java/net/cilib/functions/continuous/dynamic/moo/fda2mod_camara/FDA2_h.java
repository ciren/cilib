 ...
 *  \___/_/_/_/_.___/
 */
package net.cilib.functions.continuous.dynamic.moo.fda2mod_camara;
...
public class FDA2_h extends ContinuousFunction {
    ...
    public Double apply(int iteration, Vector x) {
        ...
        double H = Math.pow((double) z, -1.0 * Math.cos((double) Math.PI * (double) t / 4.0));
        Vector xI = x;
        Vector xII = x;
        Vector xIII = x;
        if (x.size() > 1) {
            xI = x.copyOfRange(0, 1);
            xII = x.copyOfRange(1, 16);
        ...
        for (int k = 0; k < xIII.size(); k++) {
            power += Math.pow(xIII.doubleValueOf(k) - ((double) H / 2.0), 2);
        }
