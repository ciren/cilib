 ...
 *  \___/_/_/_/_.___/
 */
package net.cilib.functions.continuous.dynamic.moo.hef3;

import net.cilib.functions.ContinuousFunction;
import net.cilib.type.types.container.Vector;

/**
 * This function is the f1 function of the adapted F3 problem defined in the following paper:
 ...
 *
 */
public class HEF3_f1 extends ContinuousFunction {
    ...
    public Double f(Vector input) {
        ...
        for (int k=2; k < input.size(); k+=2) {
            sum += Math.pow(input.doubleValueOf(k)-0.8*(double)value*Math.cos(6.0*Math.PI*value + (double)k*Math.PI/input.size()), 2);
            index++;
        }
        sum *= 2.0/index;
        sum += value;
