...
public class TetrisFeaturesInputStrategy extends NeuralStateInputStrategy {
    ...
    public Vector getNeuralInputArray(NeuralAgent currentPlayer,
        ...
        double landHeight = 0.5 * ((stateData.getCurrentShape().getBottomMostBlock().getInt(1) + 1) + (stateData.getCurrentShape().getTopMostBlock().getInt(1) + 1));
        ...
        if (removedLines) {
            input.add(currentPlayer.getScaledInput(amLinesRemoved, 0, 4));
        }
        if (erodedShapeBlocks) {
            input.add(currentPlayer.getScaledInput(erodedPieceCount, 0, 4 * 4));
        }
        if (pileHeight) {
            input.add(currentPlayer.getScaledInput((Height - 1) - HighestYCell, 0, Height - 1)); //highest block
        }
        if (this.landingHeight) {
            input.add(currentPlayer.getScaledInput((Height - 1) - landHeight, 0, Height - 1));
        }
        if (altitudeDiff) {
            input.add(currentPlayer.getScaledInput((((Height - 1) - HighestYCell) + 1) - ((Height - 1) - lowestFreeCell), 0, Height - 1)); //highest block
        }
        if (holes) {
            input.add(currentPlayer.getScaledInput(amHoleCells, 0, (Height - 1) * Width));
        }
        if (connectedHoles) {
            input.add(currentPlayer.getScaledInput(amHoles, 0, Width * (Height / 2)));
        }

        if (maxWellDepth) {
            input.add(currentPlayer.getScaledInput(maximumWellDepth, 0, Height));
        }
        if (sumWells) {
            input.add(currentPlayer.getScaledInput(wellCount, 0, Height * (Width / 2)));
        }
        if (rowTransitions) {
            input.add(currentPlayer.getScaledInput(rowTransitionsCount, 0, Height * Width));
        }
        if (columnTransitions) {
            input.add(currentPlayer.getScaledInput(columnTransitionsCount, 0, Height * Width)); //highest block
        }
        if (noBlocks) {
            input.add(currentPlayer.getScaledInput(blockCount, 0, Height * Width));
        }
        if (weightedBlocks) {
            input.add(currentPlayer.getScaledInput(weightedBlockCount, 0, MaxWeightedSum));
        }
        return input.build();
    }
