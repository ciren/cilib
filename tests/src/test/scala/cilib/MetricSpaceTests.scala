...
object MetricSpaceTest extends Spec("MetricSpace") {
  ...
  property("non-negativity") = forAll(listTuple2) {
    case (x, y) =>
  ...
  property("hamming metric space") = forAll { (x: List[Double], y: List[Double]) =>
  ...
  property("identity") = forAll(listTuple2) {
    case (x, y) =>
...
//    hamming.dist(x, x) === 0 &&
  ...
  property("symmetry") = forAll(listTuple2) {
    case (x, y) =>
