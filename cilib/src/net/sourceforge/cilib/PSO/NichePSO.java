...
public class NichePSO extends Algorithm implements OptimisationAlgorithm {
    ...
    public NichePSO() {
        ...
        mainSwarm.setPrototypeParticle(
            new DeviationDecorator(new StandardParticle(), getObservations()));
    ...
    protected void performIteration() {
        ...
        while (iterator.hasNext()) {
            ...
            double deviation =
                DeviationDecorator.extract(particle).getFitnessDeviation();

            // determine if the deviation is less than the threshold.
            if (deviation < solutionThreshold) {
                // the particle has found a solution.  Find the particles closest
                // neighbour.
                Particle particleNeighbor =
                    findClosestNeighbor(particle, mainSwarm.getTopology());
                ...
                //        gcpso.initialise();
                //
                //        // empty the gcpso. we need to empty the gcpso because we dont want
                //        // any particles in the gcpso besides the two we have found.
                //        Iterator i = gcpso.getTopology().particles();
                //        while (i.hasNext())
                //        {
                //          i.remove();
                //        }
        ...
        // make sure the iterator is dead.
        iterator = null;

        // iterate through the subswarms and perform one iteration.
        iterator = subswarms.iterator();
        while (iterator.hasNext()) {
            // get the gcpso
    ...
    private int numberOfParticles() {
        int result = mainSwarm.getTopology().getSize();

        Iterator iterator = subswarms.iterator();
        while (iterator.hasNext()) {
            GCPSO gcpso = (GCPSO) iterator.next();
            result += gcpso.getTopology().getSize();
        }
    ...
    private void absorbParticles() {
        ...
        while (iterator.hasNext()) {
            ...
            if (bestIndex != -1) {
                // the best index is not -1, therefore the particle can be absorbed into the subswarm at index bestIndex.
    ...
    private void mergeSubswarms() {
        ...
        for (int i = 0; i < subswarms.size(); i++) {
            ...
            for (int j = i + 1; j < subswarms.size(); j++) {
                ...
                double distance =
                    distance(
                        gcpso1.getBestParticle(),
                        gcpso2.getBestParticle());
                ...
                // if the radius approximately 0 then merge use the merge threshold.
                if (radius1 < Math.pow(10.0, -3.0)
                    && radius2 < Math.pow(10.0, -3.0)) {
                    // the swarms diameters are too small to make a good observation.
                    if (distance < getMergeThreshold()) {
                        // merge the subswarms.
     ...
     */
    private Particle findClosestNeighbor(
        Particle particle,
        Topology topology) {
    ...
    public double distance(Particle p1, Particle p2) {
        // get the position of particle p1
        double[] position1 = p1.getPosition();

        // get the position of particle p2
    ...
    public void setMainSwarm(PSO mainSwarm) {
        ...
        this.mainSwarm.setPrototypeParticle(
            new DeviationDecorator(new StandardParticle(), getObservations()));
        /**
     ...
     * @param problem The {@link net.sourceforge.cilib.Problem.OptimisationProblem}
     * to be solved.
     */
     ...
     * @return The {@link net.sourceforge.cilib.Problem.OptimisationProblem}
     * to be solved.
     */
     ...
     * @return A double array of length {@link net.sourceforge.cilib.Problem.OptimisationProblem#getDimension()}
    ...
    public OptimisationSolution getBestSolution() {
        ...
        while (iterator.hasNext()) {
            ...
            OptimisationSolution solution =
                (OptimisationSolution) iterator.next();

            // determine if this solution is better.
            if (solution.getFitness() < result.getFitness()) {
        ...
        if (subswarms.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                while (solutionsIterator.hasNext()) {
                    OptimisationSolution solution =
                        (OptimisationSolution) solutionsIterator.next();

                    // add the solution
                    solutions.add(solution);

                    double[] position = solution.getPosition();
                    System.out.print(this.getIterations() + " : ");
    ...
    public double[] getContribution() {
        return getBestSolution().getPosition();
    }
    ...
    public Particle getBestParticle() {
        ...
        while (iterator.hasNext()) {
            ...
            if (particle.getBestFitness() < result.getBestFitness()) {
                // update the result
                result = particle;
            }
        }
