 ...
 */
public class Graph {
	protected ArrayList vertexSet;
	protected ArrayList edgeSet;
	...
	public void addVertex(Vertex v) {
		for (ListIterator l = getVertices(); l.hasNext(); ) {
			Vertex currentVertex = (Vertex) l.next();
			if (v.getIdentity() == currentVertex.getIdentity())
				throw new GraphException("Error cannont insert vertex into graph - Unique Identity already exists");
		}
	...
	public void addEdge(Edge e) {
		if (e == null)
			throw new GraphException("Cannot add a null edge to a graph");
	...
	public Vertex getVertexAt(int index) {
		if (index > vertexSet.size()) 
			throw new GraphException("Invalid index for Graph Vertex selection: " + index);
		
		return (Vertex) vertexSet.get(index);
	}
	 ...
	 * 
	 * @param v The <code>Vertex</code> to have it's iondex returned
	...
	public Edge getEdgeBetweenVerticies(Vertex v1, Vertex v2) {
		...
		for (ListIterator l = v1.getEmanatingEdges(); l.hasNext();) {
			Edge edge = (Edge) l.next();
			if (edge.getSecondVertex() == v2)
				returnEdge = edge;
		}
		return returnEdge;
	}
	 ...
	 */
	public ListIterator getVertices(int index) {
	 ...
	 */
	public void depthFirstTraversal(PrePostVisitor visitor, Vertex startVertex) {
	 ...
	 */
	private void depthFirstTraversal(PrePostVisitor visitor, Vertex vertex, boolean [] visited) {
		if (visitor.isDone())
			return;
		...
		for (ListIterator l = vertex.getEmanatingEdges(); l.hasNext(); ) {
			Edge edge = (Edge) l.next();
	 ...
	 */
	public void breadthFirstTraversal(Visitor visitor, Vertex startVertex) {
		boolean enqueued [] = new boolean[getNumberOfVertices()];
		for (int i = 0; i < getNumberOfVertices(); i++)
			enqueued[i] = false;
		
		// TODO: Convert the ArrayList to a queue data structure
		ArrayList queue = new ArrayList();
		queue.add(startVertex);
		enqueued[indexOf(startVertex)] = true;
		
		while (!queue.isEmpty() && !visitor.isDone()) {
			Vertex vertex = (Vertex) queue.remove(0);
	...
	public void printGraphSummary() {
		...
		{ 
			...
			while (l.hasNext()) {
				...
				ListIterator e = v.getIncidentEdges();
				while (e.hasNext()) {
					System.out.print(" " + ((Edge) e.next()).toString() + "\n\t  ");
				}
				System.out.println();
			}
			
			System.out.println("  Vertices and out-going edges:");
			ListIterator m = vertexSet.listIterator(0);
			while (m.hasNext()) {
				Vertex v = (Vertex) m.next();
				System.out.print("\t" + v.toString());
				ListIterator e = v.getEmanatingEdges();
				while (e.hasNext()) {
					System.out.print("   " + ((Edge) e.next()).toString() + "\n\t  ");
				}
				System.out.println();
