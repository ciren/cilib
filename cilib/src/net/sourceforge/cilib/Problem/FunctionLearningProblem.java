...
public class FunctionLearningProblem implements OptimisationProblem {
    ...
    private Domain domain = new Domain(-1.0, 1.0);
    ...
    public double getFitness(double[] solution) {
        ...
        // the NN to function.
        if (solution.length != neuralNetwork.getNumberOfWeights()) {
        ...
        while (iterator.hasNext()) {
            ...
            exp_output[0] = function.evaluate(input);
            ...
            // range [0.0, 0.1].
            exp_output[0] = exp_output[0] / functionMaxValue;
            //      double slope = 0.05;
            //      double range = 1.0;
            ...
            neuralNetwork.getOutput(input, solution, output);

            // decrement the totalFitness.
            totalFitness -= Math.pow(exp_output[0] - output[0], 2.0);
        }

        return (double) (totalFitness / trainingSet.size());
    }
