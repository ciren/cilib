...
{
	public Formula2PlayOutAgent(int currentPlayer_,int totalPlayers_,int samples_,Game game_,Agent evaluate_)
	...
	protected double[] GetScores(State[] stateArray_)
	{
		...
		for (int i=0; i<stateArray_.length; i++)
		{
			...
			for (int j=0; j<allScores.length; j++)
				draws += allScores[j];
			draws = samples - draws;
			int loss = samples - draws - allScores[currentPlayer-1];
		
			double player1 = (1.0*loss) + (2.0*draws) + (3.0*allScores[currentPlayer-1]);
