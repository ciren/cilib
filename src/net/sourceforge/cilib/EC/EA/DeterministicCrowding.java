...
public class DeterministicCrowding extends EA implements OptimisationAlgorithm {
  ...
  protected void performIteration() {
    ...
    @SuppressWarnings("unused") int populationSize = getPopulation().getSize();

    @SuppressWarnings("unused") long lastIndex = getPopulation().getLastIndex();

    // Add all the offspring created to a vector, which will be used to add
    // the offspring to the current population.
    Vector<Individual> newIndividuals = new Vector<Individual>();
  ...
  public Collection<OptimisationSolution> getSolutions() {
  	Collection<OptimisationSolution> list = new LinkedList<OptimisationSolution>();
...
//      double[][] nicheSolution = new double[niches.size()][getOptimisationProblem().getDomain().getDimension()];
      double[][] nicheSolution = new double[niches.size()][DomainParser.getInstance().getDimension()];
...
//      double[][] nicheSolution = new double[1][getOptimisationProblem().getDomain().getDimension()];
      double[][] nicheSolution = new double[1][DomainParser.getInstance().getDimension()];
  ...
  private Vector identifyNiches(Population p) {
    ...
    Vector<Population> nicheVector = new Vector<Population>();
  ...
  protected Vector<Population> mergeNiches(Vector<Population> nicheVector) {
    ...
    for(int i=0; i<nicheVector.size()-1; i++) {
      ...
      double[] center_i = p_i.getCenter(DomainParser.getInstance().getDimension());
      ...
      for(int j=i+1; j<nicheVector.size(); j++) {
        ...
        double[] center_j = p_j.getCenter(DomainParser.getInstance().getDimension());
  ...
  //	Domain domain = Domain.getInstance();
    //double max = domain.getUpperBound().doubleValue();
  	//double max = domain.getUpperBound();
    //dValue = dValue/max;
