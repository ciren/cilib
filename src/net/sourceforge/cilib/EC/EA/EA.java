                ...
                implements OptimisationAlgorithm, ParticipatingAlgorithm
{
  ...
  public void performInitialisation() {
    ...
    while (iterator.hasNext()) {
      ...
      individual.setDimension(DomainParser.getInstance().getDimension());

      // initialise the gene value for the individual.
      for (int j = 0; j < DomainParser.getInstance().getDimension(); ++j) {
        //DomainComponent domain = problem.getDomain().getComponent(j);
      	DomainComponent domain = null;
  ...
  protected void performIteration() {
    ...
    Iterator<Individual> iterator = population.individuals();
    while (iterator.hasNext()) {
      // get the individual.
    ...
    while(newIndividuals.size() < numberOfOffspring) {
      ...
      @SuppressWarnings("unused") long index = population.getLastIndex();
...
//    }
  ...
  */
  ...
  public net.sourceforge.cilib.Type.Types.Vector getContribution() {
    Gene[] solutionGenes =  (Gene[])getBestIndividual().getChromosome();
    //double[] solution = new double[solutionGenes.length];
    net.sourceforge.cilib.Type.Types.Vector solution = new MixedVector(solutionGenes.length);
    /*for (int i=0; i<solution.length; i++) {
      solution[i] = solutionGenes[i].getGeneValue();
    }*/
    for (int i = 0; i < solution.getDimension(); i++) {
    	solution.setReal(i, solutionGenes[i].getGeneValue());
    }
    return solution;
  }
