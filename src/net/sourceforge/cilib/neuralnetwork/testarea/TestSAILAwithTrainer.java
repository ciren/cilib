...
import net.sourceforge.cilib.neuralnetwork.generic.evaluationstrategies.SAILAEvaluationStrategy;
...
public class TestSAILAwithTrainer {
	...
	//	GenericTopology topo = new GenericTopology(new FFNNStaticTopologyBuilder());
		...
		try {
			...
			data = new SAILARealData("c:\\Stefan University\\masters\\datasets\\F2.txt", 2, 60, 0, 20, 20,topo);
			data.populate();
		} catch (IOException e) {
		...
		SAILAEvaluationStrategy eval = new SAILAEvaluationStrategy(topo, 
				data, 
				err,
				trainer);
...
System.out.println("test result f(0.5) = 0.25  -->  : " + ((Double)result.get(0)).doubleValue());
