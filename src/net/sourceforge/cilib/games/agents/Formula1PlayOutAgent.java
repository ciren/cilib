...
{
	public Formula1PlayOutAgent(Game game_)
	{ super(game_); }
	...
	protected double[] GetScores(State[] stateArray_)
	{
		...
		for (int i=0; i<stateArray_.length; i++)
		{
			...
			scores[i] = (allScores[player-1]*0.6) + (draws*0.4) + (theGame.GetState().GetSize()-info.GetAverageMoves());
		}	
		return scores;
	}
}
