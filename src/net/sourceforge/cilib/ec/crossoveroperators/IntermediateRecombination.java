/*
 * IntermediateRecombination.java
 * 
 ...
 * @author otter
 *
 * Intermediate Recombination, usually used within Evolutionary Stragies, one offspring is generated by
 * using the avarage of the allele values of the parents.
 * Local or global cross-over can be used.
 * Local - two parents.
 * Global - whole population.
 * 
 * Boolean value - local of global.
 * The strategy can be determined by looking at the number of parents received.
 * 
...
public class IntermediateRecombination<E extends Entity> extends CrossOverOperator<E> {
    ...
    public Collection<E> reproduce(Collection<? extends E> parents) {
        if(parents.size() < 2)
            throw new RuntimeException("Usage error : IntermediateRecombination needs at least two parents. Received " + parents.size()+ " parents");
        ...
        if(parents.size() == 2) {   //local cross-over.
            ...
            for(int i = 0; i < offspring.get(0).getDimension(); i++) {
                if(random.nextBoolean()) {  //change allele to that of the other parent.
                    ((Vector) (offspring.get(0).get())).setReal(i,
                            ( ((Vector)((Entity)((ArrayList)parents).get(0)).get()).getReal(i)+
                            ((Vector)((Entity)((ArrayList)parents).get(1)).get()).getReal(i)) /2);
                }
            }
        } else {  //global cross-over.
        	for(int i = 0; i < offspring.get(0).getDimension(); i++) {
                //pick twee random parents vir elke parameter.
                //change allele to that of average of the paramters of the two parents.
            }
