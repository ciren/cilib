 ...
 * @author filipe
 */
public class StandardMergeOperation extends MergeOperation {
    private MergeDetection mergeDetection;
    private MergeStrategy mergeStrategy;
    
    public StandardMergeOperation() {
    ...
    public P2<PopulationBasedAlgorithm, List<PopulationBasedAlgorithm>> f(PopulationBasedAlgorithm mainSwarm, List<PopulationBasedAlgorithm> subSwarms) {
        ...
        return P.p(mainSwarm, fj.data.List.cons(
                subSwarms.filter(mergeDetection.f(subSwarms.head()))
                .foldLeft(mergeStrategy, subSwarms.head()),
