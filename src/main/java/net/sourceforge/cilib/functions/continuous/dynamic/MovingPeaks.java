...
public class MovingPeaks extends ContinuousFunction implements DynamicFunction {
    ...
    private Random movrand = new Random();// movrandseed );
    private Random movnrand = new Random();// movrandseed );
     ...
     */
    protected double lambda = movrand.nextDouble();
    ...
    */
    ...
    private double exclusionthreshold = (maxcoordinate - mincoordinate)/(2*(Math.pow(number_of_peaks, 1/getDimension())));
    ...
    public void init_peaks() 
    {
		...
		{	
		    for (i = 0; i < this.number_of_peaks; i++)
		    {
				peak[i][getDimension() + 1] = (this.maxheight - this.minheight) * this.movrand.nextDouble() + this.minheight;
			}	
		}
		else 
		{	
		...
		{	
		    for (i = 0; i < this.number_of_peaks; i++) 
		    {	
				peak[i][getDimension()] = (this.maxwidth - this.minwidth) * this.movrand.nextDouble() + this.minwidth;
		    } //for
		}
		else 
		{	
    ...
    class Peak_Function_Cone implements Peak_Function {
            ...
            public double calculate(double[] gen, int peak_number) 
            {
                    ...
                    dummy =
                            (gen[0] - peak[peak_number][0])
                                    * (gen[0] - peak[peak_number][0]);
                    for (j = 1; j < getDimension(); j++)
                            dummy += (gen[j] - peak[peak_number][j])
                                    * (gen[j] - peak[peak_number][j]);
    ...
    class Peak_Function_Twin implements Peak_Function {
            ...
            public double calculate(double[] gen, int peak_number) {
                    ...
                    dummy =
                            peak[peak_number][getDimension()
                                    + 1]
                                    - (peak[peak_number][getDimension()] * dummy);
                    maximum = dummy;
                    //System.out.println("j: "+j);
                    dummy =
                            Math.pow(gen[j] - (peak[peak_number][0] + twin_peak[0]), 2);
                    for (j = 1; j < getDimension(); j++)
                            dummy
                                    += Math.pow(
                                            gen[j] - (peak[peak_number][j] + twin_peak[0]),
                                            2);
                    dummy =
                            peak[peak_number][getDimension()
                                    + 1]
                                    + twin_peak[getDimension()
                                    + 1]
                                    - ((peak[peak_number][getDimension()]
                                            + twin_peak[getDimension()])
                                            * dummy);
	...
	public double calculate(double[] gen) 
	{
		...
		for (i = 0; i < 5; i++) {
			...
			dummy =
				basis_peak[i][getDimension()
					+ 1]
					- (basis_peak[i][getDimension()] * dummy);
			if (dummy > maximum)
				maximum = dummy;
		}
		return maximum;
