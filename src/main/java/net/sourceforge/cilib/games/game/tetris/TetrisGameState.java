...
public class TetrisGameState extends GridGameState {
    ...
    private void mergeShape(AbstractShape shape) {
        if (shape != null) {
            ...
            for (TetrisBlock B : shapeBlocks) {
                ...
                setItem(itemLoc.intValueOf(0), itemLoc.intValueOf(1), B.getClone());
            }
        }
    }
    ...
    public static int clearFullRows(GameItem state[][], int Width, int Height) {
        ...
        for (int[] key : changeKeys) {
            if (state[key[0]][key[1]] != null) {
                ...
                B.updateMovesDown();
                if (((GridLocation) (B.getLocation())).intValueOf(1) != key[1]) {
        ...
        for (TetrisBlock change : changeBlocks) {
            ...
            state[loc.intValueOf(0)][loc.intValueOf(1)] = change;
        }
        return rowsCleared;
    ...
    public boolean collides(List<TetrisBlock> blocks) {
        for (TetrisBlock B : blocks) {
            ...
            try {
                if (state[loc.intValueOf(0)][loc.intValueOf(1)] != null) //if there is a block then this one collides with it!
                {
                    return true;
                }
            } catch (Exception e) {
                throw new RuntimeException(e.getMessage() + ": x: " + loc.intValueOf(0) + ", y: " + loc.intValueOf(1));
            }
        }
        return false;
    }
