	...
	public void algorithmIteration() { // Override this method to perform the needed clustering algorithm
		...
		for (ListIterator<Ant> l = ants.listIterator(); l.hasNext(); ) {
			...
			if (!ant.isLaden() && (grid.get(ant.getCurrentRow(), ant.getCurrentCol()) != null)) {
				...
				// double f = 1.0;
				double pPickup = probabilityPickup(grid.get(ant.getCurrentRow(), ant.getCurrentCol()), ant);
			...
			else if (ant.isLaden() && (grid.get(ant.getCurrentRow(), ant.getCurrentCol()) != null)) {
				// compute f(o_i) and p_p(o_i)
				// double f = 1.0;
				double pDrop = probabilityDrop(grid.get(ant.getCurrentRow(), ant.getCurrentCol()), ant);
				randomNumber = getRandomizer().nextDouble();
				
				if (randomNumber <= pDrop) { // drop the item
					grid.set(ant.getCurrentRow(), ant.getCurrentCol(), ant.getCurrentObject());
